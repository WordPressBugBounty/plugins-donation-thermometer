...
function set_plugin_meta_dt($links, $file) {
    ...
    if ($file == $plugin) {
        return array_merge(
            $links,
            array( (sprintf( '<a href="options-general.php?page=%s">%s</a>', 'thermometer-settings', __('Settings', 'donation-thermometer') ) ),
                (sprintf( '<a href="https://wordpress.org/support/plugin/donation-thermometer/" target="_blank">%s</a>', __('Support Forum', 'donation-thermometer') ) ),
                sprintf('<a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=8NVX34E692T34" target="_blank">%s</a>', __('Buy the author a coffee', 'donation-thermometer') ) )
        );
    }
    return $links;
}
